...
def main():
    ...
    print('data_v5')
    for demo_i in demo_list:
        print(demo_i + ' /   Best testing score: ' + str('%.3f' % max(demo_pred_score[demo_i])) + ' /  max depth : ' + str(demo_pred_score[demo_i].index(max(demo_pred_score[demo_i]))+1))
    ...
    the best score:
    dataset: data_v5
    age /   Best testing score: 0.453 /  max depth : 8
    gender /   Best testing score: 0.697 /  max depth : 5
    relationship /   Best testing score: 0.488 /  max depth : 9
    '''

    '''
    #plot confusion matrix at best k of microF1
    best_depth = [8, 5, 9]
    for d_i in range(len(best_depth)):
        demo_pred = []
        clf = RandomForestClassifier(max_depth=best_depth[d_i], random_state=2018)

        demo_pred = cross_val_predict(clf, user_data, user_demo[demo_list[d_i]], cv=5)
        cnf_matrix = confusion_matrix(user_demo[demo_list[d_i]], demo_pred)
        plt.figure()
        plt.tight_layout(pad=0.4, w_pad=1.0, h_pad=1.0)
        plot_confusion_matrix(cnf_matrix, classes=class_name[demo_list[d_i]], normalize=True, title=demo_list[d_i] + ' in max depth = ' + str(best_depth[d_i]))
        plt.savefig('super_rf_' + demo_list[d_i]+'.png', format='png', dpi=1000)
        plt.show()
    '''
    ...
    #plt.savefig("super_rf_microf1.eps", format='eps', dpi=1000)
    plt.savefig("super_rf_microf1.png", format='png', dpi=1000)
    plt.show()
